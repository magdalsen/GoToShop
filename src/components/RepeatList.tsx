...
const RepeatList = () => {
  ...
  const [totalPriceWithTip, setTotalPriceWithTip] = useState<number>(0);
  ...
  const {data:list,isLoading,error}=useQuery(['listRepeat',listId.id],fetchListData);
  ...
  const formValues = useWatch({
      ...
      useEffect(()=>{
       ...
       setTotalPrice(total)
       const withTip = (total*formValues2/100)+total;
       setTotalPriceWithTip(withTip);
        }
            ...
            <div className={style.leftColumn}>
                ...
                <TableContainer>
                  <Table variant='striped' colorScheme='teal' className={style.table}>
                    <TableCaption>
                      <div>Szacowana wartość bez napiwku: {totalPrice} zł</div>
                      <div>Szacowana wartość z napiwkiem: {totalPriceWithTip} zł</div>
                    </TableCaption>
                    <Thead>
                      <Tr>
                        <Th>Produkt</Th>
